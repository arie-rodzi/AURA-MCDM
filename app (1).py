...
alpha = st.slider("⚖️ Set Alpha (α): Trade-off between ideality and average", 0.0, 1.0, 0.5, step=0.05)
p = st.selectbox("📐 Select Distance Metric (p)", options=[1, 2], index=1)
...
def generate_excel_report(alternatives, criteria, weights, norm_matrix, weighted_matrix, pis, nis, avg, distances, final_df):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        pd.DataFrame({"PIS": pis, "NIS": nis, "AVG": avg}).to_excel(writer, sheet_name="Benchmarks", index=False)
